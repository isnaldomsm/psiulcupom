...
if (!defined('KSES_CLASS_PHP5')) {
    ...
    class kses5 {
         ...
         * 	@since PHP4 OOP 0.0.1
         */
        public function AddHTML($tag = "", $attribs = array(), $whitelist = array()) {
            if (!is_string($tag)) {
        ...
        private function normalizeEntities($string) {
            ...
            $string = @preg_replace(
                            '/&amp;([A-Za-z][A-Za-z0-9]{0,19});/', '&\\1;', $string
            );

            #	Change numeric entities to valid 16 bit values

            $string = @preg_replace(
                            '/&amp;#0*([0-9]{1,5});/e', '\$this->normalizeEntities16bit("\\1")', $string
            );

            #	Change &XHHHHHHH (Hex digits) to 16 bit hex values
            $string = @preg_replace(
                            '/&amp;#([Xx])0*(([0-9A-Fa-f]{2}){1,2});/', '&#\\1\\2;', $string
            );
        ...
        private function removeBadProtocols($string) {
            ...
            while ($string != $string2) {
                ...
                $string = @preg_replace(
                                '/^((&[^;]*;|[\sA-Za-z0-9])*)' .
                                '(:|&#58;|&#[Xx]3[Aa];)\s*/e', '\$this->filterProtocols("\\1")', $string
                );
            }
