...
class Record {
    ...
    public function htmlFilter($html) {
        ...
        foreach ($tags as $tag) {
            $filter->AddHTML($tag, array('class' => true, 'style' => true));
        }
        $filter->AddHTML('img', array('src' => true, 'alt' => true, 'style' => true, 'class' => true, 'data-filename'=>true));
        $filter->AddHTML('a', array('href' => true, 'target' => true, 'class' => true, 'style' => true, 'data-target' => true, 'data-toggle' => true));
        $filter->AddHTML('pre', array('class' => true, 'style' => true));
        $filter->AddHTML('i', array('class' => true, 'style' => true));
        $filter->AddHTML('code', array('class' => true, 'style' => true));
        $filter->AddHTML('iframe', array('sandbox' => true, 'class' => true, 'width' => true, 'height' => true, 'src' => true, 'allow' => true));
        ...
        preg_match_all('%<iframe[^>]*src=["\']([^"\']*)["\'][^>]*><\/iframe>%isU', $html, $matches);
        if (!empty($matches[1])) {
